...
* Prerequisites: connect smart suit with RKK studio, and use 'livestreaming' module to set stream with json format and udp address/port corresponding to the configuration in this source code. (This function 'livestreaming' should upgrade RKK account plans (>20$/month), so if it is possible, create a program to directly analyze and record udp packet (i.e., raw IMU data) sent from smart suit would be better (Actually I failed to analyze the packet because I don't know the data head of it  :-(     ), if it possible, use 'sniff' method refer to this project https://github.com/CocytusZ/Rokoko)
