...
def summarize_results(scores, params, model_type):
    ...
    pyplot.title(model_type + '_windowSizeTest')
    pyplot.ylim([0, 100])
    pyplot.xlabel('WindowSize (in frame)')
    pyplot.ylabel('Accuracy (in %)')
    pyplot.savefig('./paramTestFigure/epoch=' + str(epoch) + '/' + model_type + '_windowSizeTest.png')
    pyplot.show()
...
def run_experiment(test_params, model_type, repeats=10):
    ...
    for param in test_params:
        print('\n---- WindowSize = %d ----' % param)
        # Load dataset
        train_x, train_y, test_x, test_y = loadDataset.get_dataset(param)
        scores = list()
        print('\nRound: %d' % repeats)
...
epoch = 50
batchSize = 64
windowSizes = [8, 16, 32, 64, 128, 256]

'''
'  Main function
'''
run_experiment(windowSizes, 'CNN')
run_experiment(windowSizes, 'LSTM')
